...
# 'supports_credentials=True' is VITAL for sessions and logins to work.
origins = [
    "http://127.0.0.1:5500",
    "http://localhost:5000",
    "https://physicschatbot.netlify.app",
    "https://physics-chatbot-frontend.onrender.com" 
]
...
@app.route('/')
def api_root():
    return jsonify({'message': 'Welcome to the Physics Chatbot API!'})
...
def get_admin_analytics():
    ...
    avg_duration_seconds = total_duration_seconds / len(sessions) if sessions else 0
    seven_days_ago = datetime.utcnow().date() - timedelta(days=6)
    message_activity = db.session.query(func.date(Message.timestamp), func.count(Message.id)).filter(func.date(Message.timestamp) >= seven_days_ago).group_by(func.date(Message.timestamp)).all()
    activity_dict = { (seven_days_ago + timedelta(days=i)).strftime('%Y-%m-%d'): 0 for i in range(7) }
    for date_obj, count in message_activity:
        activity_dict[date_obj.strftime('%Y-%m-%d')] = count
    sorted_activity = sorted(activity_dict.items())
    activity_labels = [datetime.strptime(date_str, '%Y-%m-%d').strftime('%b %d') for date_str, count in sorted_activity]
    activity_data = [count for date_str, count in sorted_activity]
    durations_in_minutes = [s.duration.total_seconds() / 60 for s in sessions]
    ...
    top_users_data = db.session.query(User.username, func.sum(UserSession.end_time - UserSession.start_time).label('total_duration')).join(UserSession).group_by(User.username).order_by(db.text('total_duration DESC')).limit(5).all()
    top_users = [{'username': username, 'total_time_seconds': duration.total_seconds()} for username, duration in top_users_data if duration]
    positive_feedback_count = Message.query.filter_by(feedback=1).count()
    negative_feedback_count = Message.query.filter_by(feedback=-1).count()
    ...
    return jsonify({
        ...
        'session_length_distribution': {'labels': list(bins.keys()), 'data': list(bins.values())},
        'top_active_users': top_users,
        'feedback_distribution': {'labels': ['Positive', 'Negative'], 'data': [positive_feedback_count, negative_feedback_count]}
    })
...
def ask():
    ...
    try:
        ...
        return jsonify({'response': bot_response_text, 'chat_id': chat_id, 'title': current_chat.title, 'bot_message_id': bot_message.id})
    except Exception as e:
        db.session.rollback()
...
@app.route('/api/rename_chat/<int:chat_id>', methods=['POST'])
@login_required
def rename_chat(chat_id):
    data = request.get_json()
    new_title = data.get('title')
...
def feedback():
    ...
    message.feedback = data.get('feedback_value')
    db.session.commit()
    return jsonify({'message': 'Feedback received.'})
